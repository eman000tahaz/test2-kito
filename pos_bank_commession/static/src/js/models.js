odoo.define('pos_driver.models', function (require) {
"use strict";
...
exports.PosModel = Backbone.Model.extend({
    initialize: function(session, attributes) {
        ...
        // Forward the 'driver' attribute on the selected order to 'selectedClient'
        function update_driver() {
            var order = self.get_order();
            this.set('selectedDriver', order ? order.get_driver() : null );
        }
        this.get('orders').bind('add remove change', update_driver, this);
        this.bind('change:selectedOrder', update_driver, this);
    ...
    models: [
    {
        ...
        model:  'res.company',
        fields: [ 'currency_id', 'email', 'website', 'company_registry', 'vat', 'name', 'phone', 'partner_id' ,'driver_id' , 'country_id', 'tax_calculation_rounding_method'],
        ...
        fields: ['name','street','city','state_id','country_id','vat','phone','zip','mobile','email','barcode','write_date','property_account_position_id'],
        domain: [['customer','=',true]],
        loaded: function(self,drivers){
            self.drivers = drivers;
            self.db.add_drivers(drivers);
        },
    },{
        model:  'res.country',
        fields: ['name'],
    ...
    // reload the list of drivers, returns as a deferred that resolves if there were
    // updated drivers, and fails if not
    load_new_drivers: function(){
        var self = this;
        var def  = new $.Deferred();
        var fields = _.find(this.models,function(model){ return model.model === 'res.partner'; }).fields;
        new Model('res.partner')
            .query(fields)
            .filter([['is_driver','=',true],['write_date','>',this.db.get_partner_write_date()]])
            .all({'timeout':3000, 'shadow': true})
            .then(function(drivers){
                if (self.db.add_drivers(drivers)) {   // check if the partners we got were real updates
                    def.resolve();
                } else {
                    def.reject();
                }
    ...
    push_and_invoice_order: function(order){
        ...
        if(!order.get_driver()){
            invoiced.reject({code:400, message:'Missing Driver', data:{}});
            return invoiced;
        }
...
exports.Order = Backbone.Model.extend({
    ...
    init_from_JSON: function(json) {
        ...
        if (json.driver_id) {
            driver = this.pos.db.get_driver_by_id(json.driver_id);
            if (!driver) {
                console.error('ERROR: trying to load a parner not available in the pos');
            }
        } else {
    ...
    export_as_JSON: function() {
        ...
        return {
            ...
            driver_id: this.get_driver() ? this.get_driver().id : false,
    ...
    /* ---- Driver / Customer --- */
    // the Driver related to the current order.
